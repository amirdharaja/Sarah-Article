...
def login(request):
    if request.user.is_authenticated:
        ...
        if not check.active:
            messages.error(request, 'OTP is successfully send to your email id. To enter the OTP to activate your account')
            return generate_otp(check.first_name, check.last_name, check.username)
...
def create_category(request):
    check = Category.objects.filter(name=request.POST['name'].title()).first()
    if check:
        messages.warning(request, 'Category already available')
        return redirect('get_my_articles')

    Category(name=request.POST['name'].title()).save()
...
def reset_password(request):
    check = Login.objects.filter(username=request.POST['email']).first()
    if not check:
    ...
    check.password = make_hash(request.POST['password'])
    check.active = False
    check.save()
    messages.success(request, 'Password resetted, Now you can login with you new password')
    return redirect('login')